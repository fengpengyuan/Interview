...
//linkedin interview
	...
	public static void printFactorsUtil(int dep, List<Integer> factors, int num, List<Integer> sol,List<List<Integer>> res){
		if(num==1){
	...
	public void combinationSumUtil(int dep, int[] candidates, int target, List<Integer> sol, List<List<Integer>> res, int cursum){
	...
	public void combinationSum2Util(int cur, int[] num, int target, boolean[] used, 
	...
	public static TreeNode LCAncestorBST(TreeNode root, TreeNode node1, TreeNode node2){
	...
	public static TreeNode LCAncestorBSTIterative(TreeNode root, TreeNode node1, TreeNode node2){
		if(root==null||node1==null||node2==null)
			return null;
		TreeNode cur=root;
		while(cur!=null){
	...
	public static TreeNode LCAncestorBT(TreeNode root, TreeNode node1, TreeNode node2){
		if(root==null)
			return null;
	...
	public static TreeNodeP LCAncestorWithParent(TreeNodeP node1, TreeNodeP node2){
	...
	public static TreeNodeP LCAncestorWithParent2(TreeNodeP node1, TreeNodeP node2){
		if(node1==null||node2==null)
			return null;
	...
	public TreeNode buildTreeUtil(int[] preorder, int beg1, int end1, int[] inorder, int beg2, int end2){
		if(beg1>end1)
			return null;
		...
		int len=index-beg2;
		root.left=buildTreeUtil(preorder, beg1+1, beg1+len, inorder, beg2, index-1);
		root.right=buildTreeUtil(preorder, beg1+1+len, end1,inorder, index+1, end2);
		return root;
	}
	...
	public void permuteUtil(int dep, int[] num, boolean[] used, List<Integer> sol, List<List<Integer>> res){
		if(dep==num.length){
			List<Integer> out=new ArrayList<Integer>(sol);
			res.add(out);			
		}
	...
	public void permuteUniqueUtil(int dep, int[] num, boolean[] used, List<Integer> sol, List<List<Integer>> res){
		if(dep==num.length){
			List<Integer> out=new ArrayList<Integer>(sol);
			res.add(out);			
		}
	...
	//given array, out array each element if the product of all the elems except itself
...
//	Find and print repeated sequences of 10 characters	
	...
	public static int numDecodings(String s) {
        ...
        for(int i=2;i<=s.length();i++){
            ...
            if(c2=='1'||(c2=='2'&&c1<='6')){
                dp[i]=dp[i]+dp[i-2];
                System.out.println("if two digits dp at i= "+i+" is "+dp[i]);
            }
        }
	 ...
	 public static void restoreIp(int dep, String s, String sol, List<String> res){
		 ...
		 for(int i=1;i<4&&i<s.length();i++){
			 if(isValidNum(s.substring(0,i))){
				 restoreIp(dep+1,s.substring(i), sol+ s.substring(0,i)+".", res);
			 }
		 }		 
	 }
...
//	    20      50      true
	    ...
	    String nums[] ={"3", "5", "4", "2", "6", "3", "0", "0", "5", "4", "8", "3"};
